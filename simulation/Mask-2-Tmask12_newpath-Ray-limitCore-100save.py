...
def write_results(results, start_strain, mean_degree, cp, timeExp):
...
for start_strain in [1, 2]:
#     print("Inside loop start_strain")
    for mean_degree in mean_degree_list:
#         print("Inside loop mean_degree")

        for cp in range(int(numExp/check_point) + 1): 
#             print("Inside loop cp")
            results_ids = []
            for i in range(check_point):
                results_ids.append(runExp.remote(i, mean_degree,num_nodes, T_list, mask_prob, start_strain))  
            results = ray.get(results_ids)
            print(len(results))
            write_results(results, start_strain, mean_degree, cp, timeExp)

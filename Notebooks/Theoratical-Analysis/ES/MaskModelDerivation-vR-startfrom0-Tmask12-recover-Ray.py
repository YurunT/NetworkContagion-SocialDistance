...
### COPY from MaskModelDerivation-vR-startfrom0-Tmask12-recover.py

"""
$time python MaskModelDerivation-vR-startfrom0-Tmask12-recover-Ray.py  -n 200000  -th 0.01 -m 0.45 -T 0.6 -tm1 0.4 -tm2 0.6 -md 2 -ns 2 -nc 40 -change 0
"""

########### Mask Model ES Analysis -- Ray ########### 
...
for mean_degree in mean_degree_list:
    results_ids.append(get_EpidemicSize.remote(mean_degree, numNodes, T_list, mask_prob))

results = ray.get(results_ids)
print(results)

for idx, mean_degree in enumerate(mean_degree_list):
    infection_size0.append(results[idx][0])
    infection_size1.append(results[idx][1])
    infection_size.append(results[idx][2])
    

######### Save the results for all Mean Degrees ########## 
print("Ray finished! Start wrting json...")
...
ExpPath = 'MaskRay_ES_Analysis_'+ change_folder +'/' + 'm' + str(mask_prob) + '_T' + "{0:.2f}".format(T) + '_tm1_' + "{0:.2f}".format(T_mask1) + '_tm2_' + "{0:.2f}".format(T_mask2) + '/' + timeExp

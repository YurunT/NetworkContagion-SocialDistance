...
for start_strain in [1, 2]:
    for mean_degree in mean_degree_list:
        for i in range(numExp):
            results_ids.append(runExp.remote(i, mean_degree,num_nodes, T_list, mask_prob))

results = ray.get(results_ids)

high_level_length = len(mean_degree_list) * numExp ###

for start_strain in [1, 2]:
    loop_1_length = (start_strain - 1) * high_level_length ###
    for idx, mean_degree in enumerate(mean_degree_list):
        loop_2_length = idx * numExp ###
        ttlEpidemicsSize = 0
        numEpidemics_1 = 0
        numEpidemics_2 = 0
        numEpidemics = 0
        ...
        for ii in range(numExp):
            fractionDic[ii] = results[int(loop_1_length + loop_2_length + ii )][0] ###
            infectedPerStDic[ii] = results[int(loop_1_length + loop_2_length + ii )][1] ###

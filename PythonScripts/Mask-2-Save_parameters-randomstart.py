...
#     plt.figure()
#     plt.plot(mean_degree_list, np.array(Prob_Emergence[1]), 'go')
#     plt.xlabel("Mean Degree")
#     plt.ylabel("Prob of Emergence")
...
# decide whether the seed wears a mask or not
...
#     Prob_Emergence[1].append(numEpidemics*1.0/(numExp))
    AvgValidSize[1].append(div(ttlEpidemicsSize*1.0, numEpidemics))
    AvgSize[1].append(ttlFrac*1.0/numExp)
    StdValidSize[1].append(np.std(Epidemics))
    infSt1[1].append(div(EpidemicsPerSt[0],numEpidemics))
    infSt2[1].append(div(EpidemicsPerSt[1],numEpidemics))
...
ExpPath = '../Mask2random_mask_Results_'+ change_folder +'/' + timeExp + '_n' + str(num_nodes) + '_e' + str(numExp)
